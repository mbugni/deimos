...
mountPersistentHome() {
  ...
  if [ -d /home/doomguy ]; then USERADDARGS="-M" ; fi
}
...
# add doomguy user with no passwd
action "Adding doomguy user" useradd \$USERADDARGS -c "Doomguy User" doomguy
passwd -d doomguy > /dev/null
usermod -aG wheel doomguy > /dev/null
